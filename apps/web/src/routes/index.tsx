...
function HomePage() {
  ...
  return (
    <SiteShell siteSettings={siteSettings}>
      <div data-home-layout="shelf">
        <ShelfHome {...homeProps} />
      </div>
      <div data-home-layout="developer">
        <DeveloperHome {...homeProps} />
      </div>
      <div data-home-layout="journal">
        <JournalHome {...homeProps} />
      </div>
    </SiteShell>
  );
}
...
function DeveloperHome({ posts, featuredPosts, siteSettings, tags, locale }: HomeViewProps) {
  const primaryPost = featuredPosts[0] ?? posts[0];
  const shownPosts = (featuredPosts.length ? featuredPosts : posts).slice(0, 3);
      ...
      <section className="border-b border-border/80 bg-muted/30">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <FeatureCard icon={<DatabaseIcon />} title="D1" body={m.feature_d1_body()} />
            <FeatureCard icon={<UploadCloudIcon />} title="R2" body={m.feature_r2_body()} />
            <FeatureCard
              icon={<WorkflowIcon />}
              title={m.feature_api_title()}
              body={m.feature_api_body()}
            />
            <FeatureCard
              icon={<ShieldCheckIcon />}
              title={m.feature_moderation_title()}
              body={m.feature_moderation_body()}
            />
          </div>
        </div>
      </section>

      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:px-8">
        ...
        <div className="min-w-0">
          {primaryPost ? (
            <article className="grid overflow-hidden rounded-lg border border-border bg-card shadow-xs lg:grid-cols-[0.45fr_0.55fr]">
              <img
                src={primaryPost.coverImage}
                alt=""
              ...
              <div className="flex flex-col justify-end p-6">
                ...
                <Link to="/blog/$slug" params={{ slug: primaryPost.slug }} className="group mt-4">
                  <h2 className="text-4xl leading-[1.02] font-semibold text-balance group-hover:text-link">
                    {primaryPost.title}
                  </h2>
                </Link>
                ...
                <PostMeta post={primaryPost} locale={locale} className="mt-6" />
              </div>
            </article>
          ) : null}
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {shownPosts.map((post) => (
              ...
              >
                ...
                <Link to="/blog/$slug" params={{ slug: post.slug }} className="group mt-8 block">
                  <h3 className="text-xl font-semibold text-balance group-hover:text-link">
                    {post.title}
                  </h3>
                </Link>
...
function JournalHome({ posts, featuredPosts, siteSettings, tags, locale }: HomeViewProps) {
    ...
    <>
      <section className="border-b border-border bg-card/45">
        <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <p className="text-sm font-semibold text-link uppercase">{m.home_eyebrow()}</p>
          <h1 className="mt-5 text-5xl leading-[1] font-semibold text-balance sm:text-7xl">
            {siteSettings.name}
          </h1>
      ...
      <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        {leadPost ? (
          <article className="border-b border-border pb-10">
            ...
            <Link to="/blog/$slug" params={{ slug: leadPost.slug }} className="group mt-5 block">
              <h2 className="text-4xl leading-tight font-semibold text-balance group-hover:text-link sm:text-5xl">
                {leadPost.title}
              </h2>
            </Link>
            ...
            <PostMeta post={leadPost} locale={locale} className="mt-7" />
          </article>
        ) : null}
        ...
        <div className="mt-10 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <Link
              ...
              to="/tags/$slug"
              params={{ slug: tag.slug }}
              className="rounded-md border border-border bg-background px-3 py-2 text-sm text-muted-foreground transition hover:border-ring/50 hover:text-foreground"
            >
              {tag.name}
            </Link>
          ))}
