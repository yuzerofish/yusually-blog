...
如果你面向中国大陆读者，建议准备自己的域名。`*.workers.dev` 在部分网络环境里可能访问不稳定；长期公开运营、高访问量或商业化站点，还需要按当地要求处理备案、接入和合规事项。
...
如果你使用的不是 Codex，把 `codex` 换成你的 agent id，例如 `cursor` 或 `gemini-cli`。
...
- 自定义域名是可选但推荐的。若主要面向中国大陆读者，避免只依赖 *.workers.dev。
...
没有域名也可以先上线，默认会得到 `*.workers.dev` 地址。建议正式公开时绑定自己的域名：
